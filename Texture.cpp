...
Texture::Texture(
	...
	auto bufferRes = util::createBuffer(
			device,
	...
	const auto stagingBuffer =
			std::make_unique<vk::raii::Buffer>(std::move(std::get<vk::raii::Buffer>(bufferRes)));
	const auto stagingBufferMemory =
			std::make_unique<vk::raii::DeviceMemory>(std::move(std::get<vk::raii::DeviceMemory>(bufferRes)));

	void *mappedMemory = stagingBufferMemory->mapMemory(
			0,
			imageSize,
			vk::MemoryMapFlags{}
	);
